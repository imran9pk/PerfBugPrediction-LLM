...
public class OracleScriptExecuteHandler extends AbstractNativeToolHandler<OracleScriptExecuteSettings, DBSObject, OracleDataSource> {
    ...
    public void fillProcessParameters(OracleScriptExecuteSettings settings, OracleDataSource arg, List<String> cmd) throws IOException {
        String sqlPlusExec = RuntimeUtils.getNativeBinaryName("sqlplus"); File sqlPlusBinary = new File(settings.getClientHome().getPath(), "bin/" + sqlPlusExec); if (!sqlPlusBinary.exists()) {
    ...
    protected List<String> getCommandLine(OracleScriptExecuteSettings settings, OracleDataSource arg) throws IOException {
        ...
        String url;
        if ("TNS".equals(conInfo.getProviderProperty(OracleConstants.PROP_CONNECTION_TYPE))) { url = conInfo.getServerName() != null ? conInfo.getServerName() : conInfo.getDatabaseName();
        }
        else {
        ...
        cmd.add(conInfo.getUserName() + "/" + conInfo.getUserPassword() + "@" + url); return cmd;
    }

...
public final class ConstraintParser {
    ...
    public static LC parseLayoutConstraint(String s) {
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            int len = part.length();
            if (len == 3 || len == 11) {   if (part.equals("ltr") || part.equals("rtl") || part.equals("lefttoright") || part.equals("righttoleft")) {
            ...
            try {
                ...
                if (c == 's') {
                    ...
                    if (ix > -1) {
                        ...
                        if (sp.startsWith("x") || sp.startsWith("y")) { (sp.startsWith("x") ? cc.getHorizontal() : cc.getVertical()).setShrinkPriority(Integer.parseInt(sp.substring(2)));
                        } else {
                ...
                if (c == 'g') {
                    ...
                    if (ix > -1) {
                        ...
                        if (c0 == 'x' || c0 == 'y') { (c0 == 'x' ? cc.getHorizontal() : cc.getVertical()).setGrowPriority(Integer.parseInt(gp.substring(2)));
                        } else {
                    ...
                    if (part.startsWith("gap")) {
                        BoundSize[] gaps = parseGaps(part); if (gaps[0] != null) {
...
return cc;
    ...
    private static BoundSize[] parseGaps(String s) {
        ...
        if (ix > -1) {
            ...
            ret[1] = parseBoundSize(gaps[0], true, true);   if (gaps.length > 1) {
                ret[3] = parseBoundSize(gaps[1], true, false);   if (gaps.length > 2) {
                    ret[0] = parseBoundSize(gaps[2], true, true);   if (gaps.length > 3) {
    ...
    private static UnitValue parseUnitValue(String s, UnitValue emptyReplacement, boolean isHor) {
        ...
        String cs = s; char c0 = s.charAt(0);
        ...
        if (oper != UnitValue.STATIC) {  String[] uvs;
            if (inline == false) {   String sub = s.substring(4, s.length() - 1).trim();
                ...
                if (uvs.length > 2) {   String last = uvs[uvs.length - 1];
            ...
            try {
                ...
                float value = numParts[0].length() > 0 ? Float.parseFloat(numParts[0]) : 1;     return new UnitValue(value, numParts[1], isHor, oper, cs);
    ...
    private static int getOper(String s) {
        ...
        for (int j = 0; j < 2; j++) {   for (int i = len - 1, p = 0; i > 0; i--) {
                char c = s.charAt(i);
                if (c == ')') {
                    p++;
                } else if (c == '(') {
    ...
    private static ArrayList<String> getRowColAndGapsTrimmed(String s) {
        ...
        ArrayList<String> retList = new ArrayList<String>(Math.max(s.length() >> 2 + 1, 3)); int s0 = 0, s1 = 0; int st = 0; for (int i = 0, iSz = s.length(); i < iSz; i++) {
            char c = s.charAt(i);
            if (c == '[') {
                s0++;
            } else if (c == ']') {
